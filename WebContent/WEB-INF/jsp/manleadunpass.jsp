...
<script type="text/javascript" src="${pageContext.request.contextPath}/jquery-easyui-1.4.4/locale/easyui-lang-zh_CN.js"></script>
    ...
    <script type="text/javascript">
     function formatterJson(value,row,Index){
      ...
      {
      	//alert(row.manreason);managerSubmitChangeState
      	var str = "<a href='${ pageContext.request.contextPath }/managerdocument_managerSubmitChangeState.action?did="+row.did+"'><input type='button' value='审核通过'></a>";  
      	return str;         
      }
      function onclickFunction(values){
    	//  alert("123")
    	 ...
    	 // alert(manreason)
    	  var url = '${ pageContext.request.contextPath }/managerdocument_managerRollChangeState.action';
    	    $.ajax({
    	        type: "post",
    	        url: url,
    	        data: {"did":values,"manreason":manreason},
    	        cache: false,
    	        async : true,//ajax为异步
    	         // dataType: "json",
    	       success: function ()
      ...
      function formatterrollback(value, row, Index)
      {
    	...
    	// onclick= Function(alert(document.getElementById('manreason').value))
    	//alert(document.getElementById("manreason").value)
    	//var str = "<a href='${ pageContext.request.contextPath }/managerdocument_managerRollChangeState.action?did="+value+"&manreason="+document.getElementById('manreason').value+"'><input type='button' onclick= Function(alert(document.getElementById('manreason').value))  value='退回'></a>";  
      	var str = "<input type='button' value='退回' onclick=onclickFunction("+value+")>";  
      	return str;         
      }
      ...
      {
      	//alert(value);
      	var str = "<input  id='manreason' style='font-family:'楷体';font-size:13px;width:140px;' name='manreason' type='text'  placeholder='请写入相应退回原因'/>";  
      	return str;         
      }
     ...
     <%@ include file="menu.jsp" %>>
      ...
      <section id="main-content" style="padding-top:35px;">
          <section class="wrapper" style="float:right; ">
						...
						<h3 style="text-align: center;">领导退回文档</h3>
                        ...
                        <!--  <table id="grid"></table>   -->
	                         ...
	                         <table class="easyui-datagrid" 
							    data-options="url:'${pageContext.request.contextPath}/managerdocument_unpass00.action',fitColumns:true,singleSelect:true, border : false,
				            rownumbers : true,pageList: [10,20,30],pagination : true,remoteSort:false,multiSort:true">
							    <thead>
									<tr>
										...
										<th data-options="field:'leadtime',width:230,formatter:formattertime,sortable:true,sorter:sort">退回时间</th>
									    <th data-options="field:'dimg',width:100,formatter:formatterdownload">下载</th>
										<th data-options="field:'a',width:120,formatter:formattercheck">审核</th>
										 <th data-options="field:'did',width:100,formatter:formatterrollback">退回</th>
										 <th data-options="field:'c',width:200,formatter:formatterrollbackreason">退回原因</th>
									     <th data-options="field:'leadreason',width:200">领导退回原因</th>
									</tr>
							    </thead>
							</table>    
